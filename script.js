...
def send_bluetooth_dht():
    ...
    if humidity !=None or temperature !=None:
        s.write("Temperature at {} is {}".format(time,temperature).encode('ascii'))
        s.write("Humidity at {} is {}".format(time,humidity).encode('ascii'))
        print('Humidity={} ; Temperature = {} '.format(humidity,temperature))
    else:
        s.write('Unable to read from sensor at time {}'.format(time).encode('ascii'))
...
try:
    while True:
        ...
        if inp in commands:
            ...
            s.write('Invalid command word'.encode('ascii'))
        time.sleep(2)
except KeyboardInterrupt:
    pass
